...
**严格禁止自行创建脚本**。本技能目录下已提供完整的工具脚本（`pdf_analyzer.py`、`pdf_split_part.py`、`save_chapters.py`、`generate_report.py`、`validate_splitbook.py`），Agent 必须直接调用这些脚本，而不是自行编写替代脚本。

- 所有 PDF 操作（校准、拆分、文本提取）必须通过 `pdf_analyzer.py` 完成
- 章节分析结果保存必须通过 `save_chapters.py` 完成
- 报告生成必须通过 `generate_report.py` 完成
- **报告质量验证**必须通过 `validate_splitbook.py` 完成
- 仅在 `pdf_analyzer.py` 无法提取 TOC 时，才使用 `pdf_split_part.py`

### 命名规范（关键）

为避免**物理分割**与**逻辑章节**混淆，文件命名遵循以下约定：

| 类型 | 命名模式 | 示例 | 说明 |
|------|----------|------|------|
| **物理分割文件** | `{书名}_split_{index:03d}_{标题}` | `{书名}_split_008_第一章-心智行为.txt` | 按TOC条目顺序分割，index从1开始 |
| **逻辑章节分析** | `chapter_{chapter_number:02d}.json` | `chapter_01.json`, `chapter_02.json` | 仅正文章节，使用书中章节编号 |

- `split_{index}`: 物理文件，按TOC原始条目顺序，从001开始
- `chapter_{number}`: 逻辑文件，使用书中实际的章节编号（如第1章→chapter_01）
- **辅助内容**（前言、附录等）：不生成 `chapter_XX.json`，可选择性生成 `aux_{index}.json`
...
- **智能细分**：仅当某个一级部分页数 > 150页时，才使用其下的二级章节进行细分
...
- `confidence` 为 `low` 或 `requires_confirmation` 为 `true` 时，**必须人工确认**：提取 `toc_page` 和 `actual_page` 两页的文本，目测哪一页包含章节标题，手动确定 offset
...
# 提取各章节文本为 .txt（单章>150页自动采样）
python pdf_analyzer.py extract_text <pdf_path> <output_dir> --offset <N>
```

`extract_text` 命令：
- 单章 ≤ 150页：全文提取
- 单章 > 150页：自动按比例采样（前20% + 中间40%中心区 + 后20%，其余页只保留段落首行）
  输出文本开头会有 `[SAMPLED]` 标记，需在子Agent prompt中告知此章为采样分析

完成后 `output_dir` 内容：
```
book_structure.json
{书名}_split_001_{标题}.pdf      ← 按TOC条目顺序的物理分割
{书名}_split_001_{标题}.txt      ← 子Agent读取这个（使用index定位）
{书名}_split_002_{标题}.pdf
{书名}_split_002_{标题}.txt
...
chapter_01.json                  ← 子Agent分析结果（使用chapter_number）
chapter_02.json
...
```

**注意**：物理文件使用 `split_{index}` 命名（如 `split_001`），逻辑章节分析结果使用 `chapter_{number}` 命名（如 `chapter_01`）。子Agent读取的是 `split_{index}.txt`，输出的是 `chapter_{number}.json`。
...
文件路径：{{txt_file}}
章节信息：第{{chapter_number}}章《{{title}}》，原书第{{start_page}}-{{end_page}}页，共{{page_count}}页
<!-- 如果章节被采样，添加以下说明 -->
{{sampled_note}}
<!-- 如果文本包含多个短章节，添加以下说明 -->
{{multi_chapter_note}}
...
【重要】章节标题格式要求：
输出的title字段必须严格与book_structure.json中的章节标题完全一致，包含"第X章"前缀。
例如：如果章节标题是"第1章 教育心理学：教学的基础"，则输出title必须是"第1章 教育心理学：教学的基础"，
不能只输出"教育心理学：教学的基础"。

输出JSON时，chapter_number字段必须与输入的章节编号一致（如第1章则chapter_number为1）。
...
输出格式：
{
  "chapter_number": {{chapter_number}},    // 使用书中的章节编号（如第1章则填1）
  "title": "{{title}}",
  "sampled": {{true|false}},
...
```bash
python save_chapters.py <output_dir>
```

通过 stdin 传入 JSON 数据（支持大参数）：
```bash
echo '{"chapter_number":1,"title":"第1章","core_question":"..."}' | python save_chapters.py ./book_output
```

或从文件读取：
```bash
python save_chapters.py ./book_output < chapter_data.json
```

示例（单章）：
```bash
echo '{"chapter_number":1,"title":"第1章 教育心理学","core_question":"..."}' | python save_chapters.py ./book_output
```

示例（多章）：
```bash
echo '[{"chapter_number":1,...},{"chapter_number":2,...}]' | python save_chapters.py ./book_output
```

此脚本会：
- 保存每个章节为 `chapter_{chapter_number}.json`（如 `chapter_01.json`）
- 自动更新 `book_structure.json` 的 `status` 和 `json_file` 字段
- 使用 `chapter_number` 而非物理 `index` 作为文件名
...
报告特性：
- **视觉设计**：简洁卡片式布局
- **思维导图**：HTML嵌套树状结构展示全书结构
- **章节详情**：可折叠的章节分析卡片
- **Sticky侧边栏导航**：快速跳转到各章节
...
```bash
python validate_splitbook.py <output_dir>
```

验证项目：
- `book_structure.json` 完整性和字段正确性
- 章节分析完成率（低于70%会警告）
- HTML报告模板变量是否全部替换
- 是否泄露本地文件路径
- 关键内容区域是否存在

验证通过后方可视为任务完成。若有警告/错误，根据提示修复后重新生成。
...
| `generate_report.py` | 从分析结果生成HTML报告 | 主Agent |
| `validate_splitbook.py` | 验证输出质量和完整性 | 主Agent |
...
├── book_structure.json         ← 结构元数据（含各章 status、chapter_number 映射）
├── {书名}_split_001_{标题}.pdf  ← 按TOC条目顺序的物理分割PDF
├── {书名}_split_001_{标题}.txt  ← 物理分割后的纯文本（子Agent输入）
├── {书名}_split_002_{标题}.pdf
├── {书名}_split_002_{标题}.txt
├── chapter_01.json             ← 第1章分析结果（子Agent输出）
├── chapter_02.json             ← 第2章分析结果
├── chapter_03.json
└── ...
```

**命名约定说明**：
- `split_{index}`: 物理分割文件，`index` 为TOC条目顺序（从001开始）
- `chapter_{number}`: 逻辑章节分析结果，`number` 为书中实际的章节编号（从01开始）
- 辅助内容（前言、附录等）不生成 `chapter_{number}.json`
...
为避免物理分割与逻辑章节混淆，系统采用分层命名：
- **物理文件**：`split_{index}` —— 按TOC条目顺序，index从1开始
- **逻辑分析**：`chapter_{number}` —— 使用书中实际章节编号（第1章→chapter_01）

`generate_report.py`通过以下策略匹配`book_structure.json`与`chapter_{number}.json`：
1. **chapter_number优先**：使用 `book_structure.json` 中的 `chapter_number` 字段匹配 `chapter_{number}.json`
2. **标题精确匹配**：比较 `title` 字段是否完全一致
3. **模糊匹配**：去除"第X章"等前缀后比较核心内容
4. **索引回退**：如没有 `chapter_number`，按物理 `index` 匹配

**子Agent必须确保**：输出的JSON中必须包含 `chapter_number` 字段，且与书中章节编号一致（如第1章则填1）。报告生成时按 `chapter_number` 排序展示。
...
| **单章 > 150页** | 自动启用采样模式（前20% + 中间40% + 后20%），子Agent收到 `[SAMPLED]` 标记 |
| **超长章节 > 150页** | 考虑按子标题手动拆分后再分析 |
...
- **报告生成后必须运行 `validate_splitbook.py` 验证输出质量**
